...
export type SeatStatus = 'available' | 'locked' | 'occupied';
...
export interface OfflineBookingDraft {
  ...
  contact_phone: string;
  total_price: number;
  passengers: Omit<Passenger, 'passport_number'>[]; // Excluding sensitive passport details for offline localStorage compliance
  flight_number: string;
  route: string;
  departure_time: string;
}
